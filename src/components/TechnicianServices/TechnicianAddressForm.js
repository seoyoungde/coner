...
const TechnicianAddressForm = ({ title, description, buttonText }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [searchParams] = useSearchParams();

  const { updateRequestData, requestData } = useRequest();
  ...
  const selectedTechnician =
    location.state?.selectedTechnician ||
    requestData.selectedTechnician ||
    (technicianId ? { id: technicianId } : null);

  useEffect(() => {
    if (!location.state?.selectedTechnician && technicianId) {
      updateRequestData("selectedTechnician", { id: technicianId });
    }
  }, [technicianId]);
  ...
  useEffect(() => {
    ...
    if (ref) {
      updateRequestData("referral_source", ref);
    }
  }, [searchParams]);
  ...
  useEffect(() => {
    if (location.state?.selectedTechnician) {
      updateRequestData(
        "selectedTechnician",
        location.state.selectedTechnician
      );
  ...
  const handleSubmit = (e) => {
    ...
    navigate(`/technician-selectservicedate/${selectedTechnician.id}`);
  };
...
`;
export default TechnicianAddressForm;
