...
import TechnicianAdditionalDropSelected from "../../components/TechnicianServices/TechnicianAdditionalDropSelected";

const TechnicianAdditionalRequest = () => {
  ...
  const selectedTechnician = requestData.selectedTechnician || {};
  ...
  const needsAdditionalDrop2Selected = ["설치&에어컨구매"].includes(
    requestData.service_type
  );
  const needsTechnicianAdditionalDropSelected = ["설치"].includes(
    requestData.service_type
  );
  ...
  const [
    selectedTechnicianDropdownOption,
    setSelectedTechnicianDropdownOption,
  ] = useState("");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      } else if (requestData.service_type === "설치&에어컨구매") {
        formattedDetailInfo =
          `${requestData.detailInfo}\n${selectedDropdownOption}\n${purchaseOption}\n${additionalInfo}`.trim();
      ...
      if (selectedTechnician?.id) {
        updateRequestData("partner_id", selectedTechnician.id);
        updateRequestData("partner_uid", selectedTechnician.id || "");
        updateRequestData("partner_name", selectedTechnician.name || "");
        updateRequestData("partner_address", selectedTechnician.address || "");
        updateRequestData(
          "partner_address_detail",
          selectedTechnician.address_detail || ""
        );
      }
      ...
      updateRequestData("purchaseOption", purchaseOption);
      ...
      const requestId = await submitRequest({
        ...
        ...(selectedTechnician && {
          partner_address: selectedTechnician.address || "",
          partner_address_detail: selectedTechnician.address_detail || "",
          partner_name: selectedTechnician.name || "",
          partner_uid: selectedTechnician.id || "",
        }),
      });

      resetRequestData();
      const partnerId = selectedTechnician.id;
      ...
      <Container ref={ref}>
        ...
        <InnerWrapper>
          ...
          >
            ...
            {needsTechnicianAdditionalDropSelected && (
              <TechnicianAdditionalDropSelected
                options={[
                  "에어컨 구매까지 원해요.",
                  "에어컨은 있어요 설치 서비스만 원해요.",
                ]}
                placeholderText="에어컨 보유 여부 선택하기"
                boxPerRow={2}
                onSelect={(option) =>
                  setSelectedTechnicianDropdownOption(option)
                }
              />
            )}
...
export default TechnicianAdditionalRequest;
